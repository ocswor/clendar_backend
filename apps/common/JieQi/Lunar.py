...
JIE_QI = {
    1: u"立春", 2: u"雨水", 3: u"惊蛰", 4: u"春分", 5: u"清明", 6: u"谷雨", 7: u"立夏", 8: u"小满", 9: u"芒种", 10: u"夏至", 11: u"小暑",
    12: u"大暑",
    13: u"立秋", 14: u"处暑", 15: u"白露", 16: u"秋分", 17: u"寒露", 18: u"霜降", 19: u"立冬", 20: u"小雪", 21: u"大雪", 22: u"冬至",
    23: u"小寒", 24: u"大寒"
}
...
# 起大运岁计算 OUT:dt-type
def eric_getQiyun_Date(dt, sex=1):
    # 默认sex=1 为男  sex=0为女
    if not isinstance(dt, datetime.datetime):
        return ""
    # 出生年干. 起位 甲乙丙丁.... 1开始
...
def getConstellation(dt):
    ...
    if dt >= Fdt(y, 1, 20) and dt <= Fdt(y, 2, 18):
        conStr = u'水瓶座'
    elif dt >= Fdt(y, 2, 19) and dt <= Fdt(y, 3, 20):
        conStr = u'双鱼座'
    elif dt >= Fdt(y, 3, 21) and dt <= Fdt(y, 4, 19):
        conStr = u'白羊座'
    elif dt >= Fdt(y, 4, 20) and dt <= Fdt(y, 5, 20):
        conStr = u'金牛座'
    elif dt >= Fdt(y, 5, 21) and dt <= Fdt(y, 6, 21):
        conStr = u'双子座'
    elif dt >= Fdt(y, 6, 22) and dt <= Fdt(y, 7, 22):
        conStr = u'巨蟹座'
    elif dt >= Fdt(y, 7, 23) and dt <= Fdt(y, 8, 22):
        conStr = u'狮子座'
    elif dt >= Fdt(y, 8, 23) and dt <= Fdt(y, 9, 22):
        conStr = u'处女座'
    elif dt >= Fdt(y, 9, 23) and dt <= Fdt(y, 10, 23):
        conStr = u'天秤座'
    elif dt >= Fdt(y, 10, 24) and dt <= Fdt(y, 11, 21):
        conStr = u'摩羯座'
    elif dt >= Fdt(y, 11, 22) and dt <= Fdt(y, 12, 21):
        conStr = u'射手座'
    else:
        conStr = u'摩竭座'
    return conStr
...
class BazhiDate():
    ...
    def xunKong_list(self):
        res = []
        for i, j in [(self.ygNum, self.yzNum), (self.mgNum, self.mzNum), (self.dgNum, self.dzNum),
                     (self.tgNum, self.tzNum)]:
    ...
    def printTianganDizhi(self):
        print(self.yg, self.yz, self.mg, self.mz, self.dg, self.dz, self.tg, self.tz, self.year)
